...
public class VendorDaoImpl implements VendorDao {
	...
	public void setDataSource(DataSource datasource) {
		...
		simpleJdbcInsert.withTableName("vendor_info");
	}

	public boolean insertVendor(Vendor vendor) {
		boolean status = false;
		String sql = "INSERT INTO vendor_info2(Name,Email,Password,File) VALUES(:name,:email,:password,:fileName)";
	...
	public boolean selectvendor(Vendor vendor) {
		boolean status = false;
		String sql = "SELECT * FROM vendor_info2 WHERE Email=:email AND Password=:password";
		BeanPropertySqlParameterSource parameters = new BeanPropertySqlParameterSource(vendor);
		Vendor v = new Vendor();
		try {
	...
	public boolean editVendor(int vid, String name, String email) {
		boolean status = false;
		int rs = 0;
		String sql = "UPDATE vendor_info2 SET Name=:name, Email=:email WHERE id=:id";
	...
	public boolean deleteVendor(int vid) {
		...
		boolean status = false;
		int rs = 0;
		String sql = "DELETE FROM vendor_info2 WHERE Id=:id";
		MapSqlParameterSource parameters = new MapSqlParameterSource();
		parameters.addValue("id", vid);
		rs = jdbcTemplate.update(sql, parameters);
		if (rs != 0) {
	...
	public Vendor selectvendorById(int vid) {
		boolean status = false;
		String sql = "SELECT * FROM vendor_info2 WHERE ID=:id";
		Vendor vendor = new Vendor();
		vendor.setId(vid);
		Vendor v = new Vendor();
		MapSqlParameterSource parameters = new MapSqlParameterSource();
